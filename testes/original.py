...
class PitagorasBloquinhos(Scene):
    def construct(self):
        ...
        # Quadrados nos lados
        sq_a = Square(side_length=a, color=BLUE).next_to(tri, DOWN, buff=0, aligned_edge=LEFT)
        sq_b = Square(side_length=b, color=GREEN).next_to(tri, LEFT, buff=0, aligned_edge=DOWN)
        sq_c = Square(side_length=c, color=RED).move_to([a/2, b/2, 0]).rotate(-np.arctan(b/a))
